...
class TransformerRegressor(nn.Module):
    def __init__(self, input_dim, output_dim, d_model=32, nhead=8, num_encoder_layers=6, num_decoder_layers=6,
...
def inject_attack(y_true, attack_type="random_multiplicative", ratio=0.8, magnitude=50):
    """
    输入 y_true（原始“干净”的观测，shape=(n_samples, n_features)），
    返回 (y_observed, attack_labels)：
      - y_observed: 含攻击的观测（被篡改的）
      - attack_labels: 0/1阵列，1=被攻击
    """
    y_attack = y_true.copy()
    n_samples = len(y_true)
    n_attack = int(ratio * n_samples)
    attack_idx = np.random.choice(n_samples, n_attack, replace=False)

    if attack_type == "voltage_shift":
        y_attack[attack_idx, 0] += magnitude
    elif attack_type == "current_shift":
        y_attack[attack_idx, 1] += magnitude
    elif attack_type == "random_noise":
        y_attack[attack_idx] += np.random.normal(0, magnitude, size=y_attack[attack_idx].shape)
    elif attack_type == "random_multiplicative":
        factors = np.arange(0.8, 1.3, 0.0015)
        for idx in attack_idx:
            random_factor = np.random.choice(factors)
            y_attack[idx, 0] *= random_factor
            y_attack[idx, 1] *= random_factor

    labels = np.zeros(n_samples, dtype=int)
    labels[attack_idx] = 1
    return y_attack, labels

def get_thresholds(model, X_valid_tensor, y_valid_tensor, scaler_y):
    model.eval()
    with torch.no_grad():
        preds = model(X_valid_tensor)
    preds_np = preds.cpu().numpy()
    preds_original = scaler_y.inverse_transform(preds_np)
    y_valid_original = scaler_y.inverse_transform(y_valid_tensor.cpu().numpy())

    err_voltage = np.abs(y_valid_original[:, 0] - preds_original[:, 0])
    err_current = np.abs(y_valid_original[:, 1] - preds_original[:, 1])

    voltage_th = 1.5 * np.percentile(err_voltage, 95)
    current_th = 1.5 * np.percentile(err_current, 95)

    return voltage_th, current_th

def detect_and_evaluate(model, X_test_tensor, y_test_observed, voltage_th, current_th, attack_labels, scaler_y):
    """
    y_test_observed: 测试时观测到的值（可能包含攻击，原始尺度）
    attack_labels: 0/1 向量
    """
    model.eval()
    with torch.no_grad():
        preds = model(X_test_tensor)
    preds_np = preds.cpu().numpy()
    preds_original = scaler_y.inverse_transform(preds_np)  # 模型预测回到原始尺度

    # 误差是 模型预测 vs 当前观测（包含被篡改的观测）
    err_voltage = np.abs(y_test_observed[:, 0] - preds_original[:, 0])
    err_current = np.abs(y_test_observed[:, 1] - preds_original[:, 1])

    detected = ((err_voltage > voltage_th) | (err_current > current_th)).astype(int)

    f1 = f1_score(attack_labels, detected)
    precision = precision_score(attack_labels, detected)
    recall = recall_score(attack_labels, detected)

    # 额外：混淆矩阵与 FPR / TPR（TPR == recall）
    tn, fp, fn, tp = confusion_matrix(attack_labels, detected).ravel()
...
results = {}
# 准备干净的测试观测（原始尺度）
y_test_clean = scaler_y.inverse_transform(y_test_tensor.cpu().numpy())

for model_name, model in models.items():
    print(f"\n[模型: {model_name}]")

    # 用验证集计算阈值（不包含攻击）
    v_th, c_th = get_thresholds(model, X_valid_tensor, y_valid_tensor, scaler_y)

    # 在测试集的一半注入攻击（ratio=0.5）
    y_test_observed, attack_labels = inject_attack(y_test_clean, attack_type="random_multiplicative", ratio=0.5, magnitude=30)

    # 在整个测试集上检测并评估（正例是 attack_labels==1）
    metrics = detect_and_evaluate(model, X_test_tensor, y_test_observed, v_th, c_th, attack_labels, scaler_y)

    results[model_name] = metrics
    print(f"F1={metrics['F1']:.4f}, Precision={metrics['Precision']:.4f}, Recall={metrics['Recall']:.4f}, TPR={metrics['TPR']:.4f}, FPR={metrics['FPR']:.4f}")

# 打印汇总
print("\n=== 攻击检测结果对比 ===")
for m, r in results.items():
    print(f"{m}: F1={r['F1']:.4f}, Precision={r['Precision']:.4f}, Recall={r['Recall']:.4f}, TPR={r['TPR']:.4f}, FPR={r['FPR']:.4f}")
