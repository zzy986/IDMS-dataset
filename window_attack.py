...
for idx in df[mask_random].index:
    random_factor = np.random.choice(np.arange(0.5, 1.3, 0.05))  # 生成 [0.1, 0.7] 范围内的随机数
    print(random_factor)
...
df.loc[mask_over, column_to_modify] = 1.3*df.loc[mask_over, column_to_modify]
df.loc[mask_over, column_to_modify_1] = 2*df.loc[mask_over, column_to_modify_1]
