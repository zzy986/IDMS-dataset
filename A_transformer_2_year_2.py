...
class CNN_LSTMRegressor(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, num_layers=2, dropout=0.3, conv_channels=32, kernel_size=3):
        super(CNN_LSTMRegressor, self).__init__()
        self.hidden_dim = hidden_dim
        self.num_layers = num_layers

        # 卷积层 (输入: [batch, seq_len, input_dim] -> [batch, conv_channels, seq_len])
        self.conv1d = nn.Conv1d(in_channels=input_dim, out_channels=conv_channels, kernel_size=kernel_size, padding=kernel_size//2)
        self.relu = nn.ReLU()
        self.bn = nn.BatchNorm1d(conv_channels)

        # LSTM层 (输入: [batch, seq_len, conv_channels])
        self.lstm = nn.LSTM(conv_channels, hidden_dim, num_layers, batch_first=True, dropout=dropout)
    ...
    def forward(self, x):
        ...
        # 先卷积 (需要转为 [batch, input_dim, seq_len])
        x = x.permute(0, 2, 1)  # [batch, input_dim, seq_len]
        x = self.conv1d(x)      # [batch, conv_channels, seq_len]
        x = self.bn(x)
        x = self.relu(x)
...
'Transformer': TransformerRegressor(input_dim=X_train_tensor.shape[2], output_dim=y_train_tensor.shape[1]).to(
        device),
'CNN_LSTM': CNN_LSTMRegressor(input_dim=X_train_tensor.shape[2], hidden_dim=128, output_dim=y_train_tensor.shape[1]).to(device),
'GRU': GRURegressor(input_dim=X_train_tensor.shape[2], hidden_dim=256, output_dim=y_train_tensor.shape[1]).to(device),
'RNN': RNNRegressor(input_dim=X_train_tensor.shape[2], hidden_dim=256, output_dim=y_train_tensor.shape[1]).to(
        device),
}
...
def train_and_evaluate_model(model, train_loader, valid_loader, test_loader, optimizer, criterion):
    ...
    with tqdm.tqdm(total=epochs, unit="Epoch") as tepoch:
        for epoch in range(epochs):
            ...
            tepoch.update()
            tepoch.set_postfix({"Train": train_losses[-1], "Valid": valid_loss_avg})
...
def save_losses(results, train_csv="training_losses_mitigation.csv", valid_csv="validation_losses_mitigation.csv"):
    # === 保存训练损失 ===
    if os.path.exists(train_csv):
        df_train = pd.read_csv(train_csv, index_col="Epoch")
    else:
        df_train = pd.DataFrame()

    # 当前训练中最长的 epoch
    max_train_len = max(len(r['Train Loss']) for r in results.values())
    new_max_len = max(max_train_len, len(df_train))  # 取历史和当前的最大长度

    # 先补齐已有列
    for col in df_train.columns:
        if len(df_train[col]) < new_max_len:
            df_train[col] = list(df_train[col]) + [None] * (new_max_len - len(df_train[col]))

    # 更新/新增本次模型
    for model_name, r in results.items():
        train_loss = r['Train Loss'] + [None] * (new_max_len - len(r['Train Loss']))
        df_train[model_name] = train_loss
    ...
    if os.path.exists(valid_csv):
        df_valid = pd.read_csv(valid_csv, index_col="Epoch")
    else:
        df_valid = pd.DataFrame()

    # 当前训练中最长的 epoch
    max_valid_len = max(len(r['Valid Loss']) for r in results.values())
    new_max_len = max(max_valid_len, len(df_valid))  # 取历史和当前的最大长度

    # 先补齐已有列
    for col in df_valid.columns:
        if len(df_valid[col]) < new_max_len:
            df_valid[col] = list(df_valid[col]) + [None] * (new_max_len - len(df_valid[col]))

    # 更新/新增本次模型
    for model_name, r in results.items():
        valid_loss = r['Valid Loss'] + [None] * (new_max_len - len(r['Valid Loss']))
        df_valid[model_name] = valid_loss

    df_valid.index = range(1, len(df_valid) + 1)
    df_valid.index.name = "Epoch"
    df_valid.to_csv(valid_csv)
    print("验证损失已追加/更新到", valid_csv)
